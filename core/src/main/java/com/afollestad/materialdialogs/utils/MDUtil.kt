...
object MDUtil {
  ...
  fun resolveLayoutDirection(
    ...
    try {
      if(attrRes != null && attrRes != 0) {
        val dir = resolveInt(context = context, attr = attrRes, defaultValue = -1)
        if(dir != -1)
        return dir != ViewCompat.LAYOUT_DIRECTION_LTR
      }
  ...
  fun resolveColor(
    ...
    return if (res == null) 0 else ContextCompat.getColor(context, res)
  }
  ...
  @RestrictTo(LIBRARY_GROUP) fun TextView?.maybeSetTextColor(
    ...
    if (this == null || (attrRes == null && hintAttrRes == null && colorRes == null)) return
    when {
      colorRes != null -> resolveColor(context, res = colorRes)
          .ifNotZero(this::setTextColor)
      attrRes != null -> resolveColor(context, attr = attrRes)
          .ifNotZero(this::setTextColor)
    }
    if (hintAttrRes != null) {
