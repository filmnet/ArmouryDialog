...
class DialogLayout(
  ...
  fun attachDialog(dialog: MaterialDialog) {
    ...
    isRtl = resolveLayoutDirection(
        dialog.context,
        attrs = attrs,
        attrArray =  R.styleable.DialogLayout,
        attrRes = R.attr.md_layout_direction
    )
    titleLayout.isRtl = isRtl
    buttonsLayout?.isRtl = isRtl
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1)
    contentLayout.layoutDirection = if(isRtl) View.LAYOUT_DIRECTION_RTL else View.LAYOUT_DIRECTION_LTR
  }
